...
class ScoreModel(L.LightningModule):
    ...
    def get_ode_sampler(
        self, y, Y_prior=None, N=None, minibatch=None, timestep_type=None, **kwargs
    ):
        N = self.sde.N if N is None else N
        sde = self.sde.copy()
        sde.N = N
        ...
        if minibatch is None:
            return sampling.get_ode_sampler(
                sde, self, y=y, Y_prior=Y_prior, timestep_type=timestep_type, **kwargs
            )
        else:
            M = y.shape[0]
    ...
    def get_prior(self, y, x, n, T=1):
        ...
        vec_t = torch.ones(Y.shape[0], device=Y.device) * torch.tensor(
            [T], device=Y.device
        )
...
class DiscriminativeModel(ScoreModel):
    ...
    def enhance(self, y, timeit=False, **ignored_kwargs):
        ...
        if self.data_module.return_time:
            ...
            Y = torch.unsqueeze(
                self._forward_transform(self._stft(y.cuda())), 0
            )  # 1,D,F,T
            Y = pad_spec(Y)
        X_hat = self(Y)
